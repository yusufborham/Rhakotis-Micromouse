...
#define GyroMeasDrift PI*(2.0f / 180.0f)
...
void IMUInit() {
  ...
  int error = imu.init(calibrationData, 0x68);
  if (error != 0) {
  ...
  Serial.println("Mag offsets X/Y/Z: ");
  ...
  delay(1000);
  imu.init(calibrationData, 0x68);

  error = imu.setGyroRange(500);
  error = imu.setAccelRange(2);
...
void IMUUpdate() {
  magnetometer.getHeading(&magX, &magY, &magZ);
  imu.update();
  ...
  MadgwickQuaternionUpdate(accelData.accelX, accelData.accelY, accelData.accelZ, gyroData.gyroX * M_PI / 180.0f, gyroData.gyroY * M_PI / 180.0f, gyroData.gyroZ * M_PI / 180.0f, magY, -magX, -magZ);
